<div>
    ...
    @foreach ($admissions as $index => $admission)
        <div class="grid grid-cols-2 ml-6 gap-6 mb-4">
            <div>
                <label for="diagnosis-{{ $index }}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Diagnosis</label>
                <input type="text" id="diagnosis-{{ $index }}"
                    wire:model.live="admissions.{{ $index }}.diagnosis"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Fever, Cough Etc." required />
            </div>

            <div>
                <label for="date-{{ $index }}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">When?</label>
                <div class="relative">
                    ...
                    <input type="date" id="date-{{ $index }}"
                        wire:model.live="admissions.{{ $index }}.date"
